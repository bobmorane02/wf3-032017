...
if (!empty($_POST) && !empty($_POST['message'])){
    ...
    // 3- Traitement contre les failles XSS (injection JS) ou le injections CSS : on parle d'échappement des données reçues
    ...
    // <script>while(true){alert('Site bloqué');}</script>

    $_POST['pseudo'] = htmlspecialchars($_POST['pseudo'],ENT_QUOTES);
    $_POST['message'] = htmlspecialchars($_POST['message'],ENT_QUOTES); // htmlspecialchars convertit les caractéres spéciaux (<,>,',",&) en
                                                                        // HTML (par exemple < devient &lt), ce qui permet de rendre inoffensives les balises
                                                                        // HTML. ENT_QUOTES permet d'ajouter les quotes à la liste des caractéres convertis.
    ...
    // 1- Nous allons faire une premiére requête non protégée contre les injections et qui n'accepte pas les apostrophes
//  $r = $pdo->query("INSERT INTO commentaire (pseudo,date_enregistrement,message) VALUES ('$_POST[pseudo]',NOW(),'$_POST[message]')");
    // 2- Nous allons faire une injection SQL : dans le message saisir "ok'); DELETE FROM commentaire; ("
